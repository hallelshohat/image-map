...
function App() {
  // Viewport tracks the visible window in world-pixel space.
  ...
  const currentLayer = useMemo(() => layerForViewport(viewport), [viewport])
  ...
  // Load a low-res preview texture once GL is ready. This keeps the screen filled while tiles stream in.
  useEffect(() => {
    const ctx = glRef.current
    if (!ctx) return
    let cancelled = false
    fetchBitmap({ x0: 0, x1: BASE_WIDTH, y0: 0, y1: BASE_HEIGHT }, 1)
      .then((bitmap) => {
        if (cancelled) {
          bitmap.close()
          return
  ...
  // Draw preview + tiles whenever viewport, tiles, or canvas size changes.
  const renderScene = useCallback(() => {
    ...
    if (previewRef.current) {
      drawQuad(
        ...
        {
          u0: viewport.x / BASE_WIDTH,
          v0: viewport.y / BASE_HEIGHT,
          u1: (viewport.x + viewport.width) / BASE_WIDTH,
          v1: (viewport.y + viewport.height) / BASE_HEIGHT,
        }
      )
    }
  ...
  }, [activeTiles, size, viewport, tileCache])

  useEffect(() => {
    renderScene()
  }, [renderScene, tileVersion, dirty, size])

  const clampViewport = useCallback((next: Viewport) => {
    const width = clamp(next.width, MIN_VIEW_WIDTH, BASE_WIDTH)
    const height = clamp(next.height, MIN_VIEW_HEIGHT, BASE_HEIGHT)
    const xMax = BASE_WIDTH - width
    const yMax = BASE_HEIGHT - height
    return {
  ...
  const handleWheel = (event: ReactWheelEvent<HTMLCanvasElement>) => {
    ...
    setViewport((prev) => {
      const nextWidth = clamp(prev.width * zoomFactor, MIN_VIEW_WIDTH, BASE_WIDTH)
      const nextHeight = clamp(prev.height * zoomFactor, MIN_VIEW_HEIGHT, BASE_HEIGHT)
